...
round(cor(df.select),4)  #correlacion con la funcion cor(matrix)

pairs(~ Price + Food + Decor + Service + East, #para graficar
...
las betas son coeficientes de regresión"
attach(df) #enmascara los campos para no poner df$campo
m1 <- lm(Price ~ Food + Decor + Service + East) #lm linear model

summary(m1)
"lo que interesa es R2 (Multiple RSquared) y Adjusted RSquared"
...
m2 <- update(m1, ~.-Service) #se actualiza, el modelo, variable a quitar de que modelo (~.-)
summary(m2)
